...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.contains(key)) {
            throw new BPlusTreeException("Can not have duplicate keys!");
        }

        int d = metadata.getOrder();

        // Insert key without overflow
        int insertIndex = InnerNode.numLessThan(key, keys);
        keys.add(insertIndex, key);
        rids.add(insertIndex, rid);

        // Check if inserting key overflows
        // If yes, split the leaf into two
        if (keys.size() > 2 * d) {
            List<DataBox> keyLeft = keys.subList(0, d);
            List<DataBox> keyRight = keys.subList(d, keys.size());
            List<RecordId> ridLeft = rids.subList(0, d);
            List<RecordId> ridRight = rids.subList(d, rids.size());
            LeafNode right = new LeafNode(metadata, bufferManager, keyRight, ridRight, rightSibling, treeContext);
            long rightPage = right.getPage().getPageNum();

            this.keys = keyLeft;
            this.rids = ridLeft;
            this.rightSibling = Optional.of(rightPage);

            sync();
            return Optional.of(new Pair<>(keyRight.get(0), rightPage));
        }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext()) {
            ...
            if (keys.size() < Math.round(2 * d * fillFactor)) {
                // Insert keys if there is still place left.
                keys.add(key);
                rids.add(rid);
                ...
                LeafNode right = new LeafNode(metadata, bufferManager, keys, rids, Optional.empty(), treeContext);
                long rightPage = right.getPage().getPageNum();
                this.rightSibling = Optional.of(rightPage);

                sync();
                return Optional.of(new Pair<>(keys.get(0), rightPage));
            }
        }
