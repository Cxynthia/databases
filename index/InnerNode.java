...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        BPlusNode targetChild = getChild(pos);
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        // Insert the new data to the leaf node first.
        int pos = numLessThanEqual(key, keys);
        BPlusNode targetChild = getChild(pos);
        Optional<Pair<DataBox, Long>> insertResult = targetChild.put(key, rid);

        // Check the return value to see if the leafNode split after the insertion.
        if (!insertResult.equals(Optional.empty())) {
            // If the leafNode split
            int d = metadata.getOrder();
            keys.add(pos, insertResult.get().getFirst());
            children.add(pos + 1, insertResult.get().getSecond());
            if (keys.size() > 2 * d) {
                List<DataBox> keyLeft = keys.subList(0, d);
                List<DataBox> keyRight = keys.subList(d + 1, keys.size());
                List<Long> childrenLeft = children.subList(0, d + 1);
                List<Long> childrenRight = children.subList(d + 1, children.size());
                DataBox keyMiddle = keys.get(d);

                InnerNode newRight = new InnerNode(metadata, bufferManager, keyRight,
                        childrenRight, treeContext);

                this.keys = keyLeft;
                this.children = childrenLeft;

                sync();
                return Optional.of(new Pair<>(keyMiddle, newRight.getPage().getPageNum()));
            } else {
                sync();
                return Optional.empty();
            }
        } else {
            // If the leafNode didn't split
            return Optional.empty();
        }
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> loadResult = getChild(keys.size()).bulkLoad(data, fillFactor);

            if (!loadResult.equals(Optional.empty())) {
                ...
                if (keys.size() > 2 * d) {
                    ...
                    InnerNode newRight = new InnerNode(metadata, bufferManager, keyRight, childrenRight, treeContext);
