...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanAll() {
        ...
        LeafNode leftMostChild = root.getLeftmostLeaf();
        Iterator<RecordId> leftMostIter = leftMostChild.scanAll();

        // TODO(hw4_part2): B+ tree locking

        return new BPlusTreeIterator(leftMostChild, leftMostIter);
    }
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        Iterator<RecordId> startIter = startLeaf.scanGreaterEqual(key);

        // TODO(hw4_part2): B+ tree locking

        return new BPlusTreeIterator(startLeaf, startIter);
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> insertResult = root.put(key, rid);

        if (insertResult.isPresent()) {
            // If the root splits
            Pair<DataBox, Long> returnPair = insertResult.get();

            List<DataBox> newKey = new ArrayList<>();
            newKey.add(returnPair.getFirst());

            List<Long> newChildren = new ArrayList<>();
            newChildren.add(root.getPage().getPageNum()); // Adding the old root as left child
            newChildren.add(returnPair.getSecond()); // Adding the returned node to be the new right child

            BPlusNode newRoot = new InnerNode(metadata, bufferManager, newKey, newChildren, lockContext);
            updateRoot(newRoot);
        }
        // If no return value, do nothing
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator(LeafNode startLeaf, Iterator<RecordId> startIter) {
            assert(startLeaf != null);
            currLeaf = startLeaf;

            assert(startIter != null);
            currIter = startIter;
