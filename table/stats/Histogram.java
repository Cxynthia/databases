...
public class Histogram {
    ...
    public void buildHistogram(Table table, int attribute) {
        ...
        minValue = quantization(iter.next(), attribute);
        maxValue = minValue;

        while (iter.hasNext()) {
            float currValue = quantization(iter.next(), attribute);
        ...
        width = (maxValue - minValue) / numBucket;
        ...
        for (Record r : table) {
            float currValue = quantization(r, attribute);
            buckets[bucketIndex(currValue)].increment(currValue);
        }
    ...
    private float [] allEquality(float qvalue) {
        ...
        // TODO(hw3_part2): implement
        int bucket = bucketIndex(qvalue);
        for (int i = 0; i < result.length; i++) {
            if (i == bucket && buckets[i].getDistinctCount() > 0) {
