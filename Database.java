...
//        recoveryManager = new DummyRecoveryManager();
    ...
    void lockTableMetadata(String tableName, LockType lockType) {
        ...
        LockContext context = getTableInfoContext();

        if (mayNeedToCreate) {
            // TODO(hw4_part2): acquire all locks needed on database/information_schema.tables before compute()
            LockUtil.ensureSufficientLockHeld(context, LockType.IX);
        ...
        RecordId id = tableInfoLookup.get(tableName);
        LockUtil.ensureSufficientLockHeld(context.childContext(id.getPageNum()), lockType);
    }
    ...
    void lockIndexMetadata(String indexName, LockType lockType) {
        ...
        LockContext context = getIndexInfoContext();
        ...
        RecordId id = indexInfoLookup.get(indexName);
        LockUtil.ensureSufficientLockHeld(context.childContext(id.getPageNum()), lockType);
    }
...
//                        if (context.getExplicitLockType(transactionContext) !=)
...
//                lockTableMetadata(prefixedTableName, LockType.NL);
        ...
        public void dropAllTables() {
            ...
            try {
                ...
                LockContext lc = getTableContext(tableNames.get(0)).parentContext();
                LockUtil.ensureSufficientLockHeld(lc, LockType.X);
