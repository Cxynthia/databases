...
public class SortOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        Iterator[] iter = new Iterator[runs.size()];

        int index = 0;
        for (Run r : runs) {
            Iterator<Record> currIter = r.iterator();
            iter[index] = currIter;
            if (currIter.hasNext()) {
                pq.add(new Pair<Record, Integer>(currIter.next(), index));
            }
            index++;
        }
        ...
        while (!pq.isEmpty()) {
            Pair<Record, Integer> curr = pq.poll();
            run.addRecord(curr.getFirst().getValues());
            int currIndex = curr.getSecond();
            Iterator<Record> currIter = iter[currIndex];

            if (currIter.hasNext()) {
                pq.add(new Pair<Record, Integer>(currIter.next(), currIndex));
            }
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        for (Run r : runs) {
            if (index == num) {
                result.add(mergeSortedRuns(group));
                group = new ArrayList<>();
                index = 1;
            } else {
                index++;
    ...
    public String sort() {
        ...
        BacktrackingIterator<Record> iter = transaction.getRecordIterator(tableName);

        while (iter.hasNext()) {
            Run run = createRunFromIterator(iter);
            sorted.add(sortRun(run));
        }
//        while (iter.hasNext()) {
//            Run currRun = this.createRun();
//            List<Record> records = new ArrayList<>();
//            Iterator<Record> pageIter = transaction.getBlockIterator(tableName, iter, numBuffers - 1);
//
//            while (pageIter.hasNext()) {
//                records.add(pageIter.next());
//            }
//            currRun.addRecords(records);
//
//            sorted.add(sortRun(currRun));
//        }
        ...
        Run sortedRun = sorted.get(0);
        return sortedRun.tableName(); // TODO(hw3_part1): replace this!
    }
