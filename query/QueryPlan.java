...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        Set startKey = new HashSet(){{add(startTableName);}};
        QueryOperator startOp = minCostSingleAccess(startTableName);
        map.put(startKey, startOp);

        for (String t : joinTableNames) {
            QueryOperator op = minCostSingleAccess(t);
            Set key = new HashSet();
            key.add(t);
            map.put(key, op);
        }

        Map<Set, QueryOperator> pass1 = map;
        ...
        addGroupBy();
        addProjects();
        return finalOperator.execute(); // TODO(hw3_part2): Replace this!!! Allows you to test intermediate functionality
    }
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        minOp = new SequentialScanOperator(this.transaction, table);
        int minColumn = Integer.MIN_VALUE;
        ...
        for (int column : getEligibleIndexColumns(table)) {
            String columnName = selectColumnNames.get(column);
            DataBox val = selectDataBoxes.get(column);
            PredicateOperator op = selectOperators.get(column);
            QueryOperator currOp = new IndexScanOperator(transaction, table, columnName, op, val);
        ...
        QueryOperator select = addEligibleSelections(minOp, minColumn);
    ...
    Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
        ...
        for (Set s : prevMap.keySet()) {
            ...
            for (int i = 0; i < joinTableNames.size(); i++) {
                ...
                String lTable = left[0];
                String lColumn = left[1];

                String[] right = getJoinRightColumnNameByIndex(i);
                String rTable = right[0];
                String rColumn = right[1];

                QueryOperator currOp = prevMap.get(s);
                QueryOperator resultOp;
                Set<String> resultTable = new HashSet<>();
                Set addKey = new HashSet();
                ...
                if (s.contains(lTable) && !s.contains(rTable)) {
                    for (Set c : pass1Map.keySet()) {
                        if (c.contains(rTable)) {
                            addKey = c;
                            break;
                        }
                    }
                    QueryOperator addOp = pass1Map.get(addKey);
                    resultOp = minCostJoinType(currOp, addOp, lColumn, rColumn);
                } else if (!s.contains(lTable) && s.contains(rTable)) {
                    for (Set c : pass1Map.keySet()) {
                        if (c.contains(lTable)) {
                            addKey = c;
                            break;
                        }
                    }
                    QueryOperator addOp = pass1Map.get(addKey);
                    resultOp = minCostJoinType(addOp, currOp, lColumn, rColumn);
                } else {
                    continue;
                }

                resultTable.addAll(s);
                resultTable.addAll(addKey);

                int lastCost = Integer.MAX_VALUE;
                int prevCost = Integer.MAX_VALUE;
                int currCost = resultOp.estimateIOCost();

                if (prevMap.containsKey(resultTable)) {
                    prevCost = prevMap.get(resultTable).estimateIOCost();
                }

                if (map.containsKey(resultTable)) {
                    lastCost = map.get(resultTable).estimateIOCost();
                }

                if (currCost < prevCost && currCost < lastCost) {
                    map.put(resultTable, resultOp);
                }
            }
        }
